...
func (sd sqlDB) GetDeviceBlockNum(deviceID string) (int64, error) {
	...
	var count int64
	cmd := fmt.Sprintf("SELECT count(id) FROM %s WHERE device_id=? AND fid != 0 ;", fmt.Sprintf(blockInfoTable, area))
	...
	if createBlocks != nil {
		for _, info := range createBlocks {
			if info.ID != "" {
				...
				cmd := fmt.Sprintf(`INSERT INTO %s (cache_id, carfile_id, cid, device_id, status, size, reliability, id, fid) VALUES (?, ?, ?, ?, ?, ?, ?, REPLACE(UUID(),"-",""), ?)`, bTableName)
				tx.MustExec(cmd, info.CacheID, info.CarfileID, info.CID, info.DeviceID, info.Status, info.Size, info.Reliability, info.FID)
			}
		}
	}
...
func (sd sqlDB) SetBlockInfos(infos []*BlockInfo, carfileCid string) error {
	...
	for _, info := range infos {
		if info.ID != "" {
			...
			cmd := fmt.Sprintf(`INSERT INTO %s (cache_id, carfile_id, cid, device_id, status, size, reliability, id) VALUES (?, ?, ?, ?, ?, ?, ?, REPLACE(UUID(),"-",""))`, tableName)
...
func (sd sqlDB) GetNodesWithCache(cid string, isSuccess bool) ([]string, error) {
	...
	if isSuccess {
		cmd = fmt.Sprintf(`SELECT * FROM %s WHERE cid=:cid AND fid != 0`, fmt.Sprintf(blockInfoTable, area))
	}
...
func (sd sqlDB) GetNodesFromData(cid string) (int, error) {
	...
	cmd := fmt.Sprintf("SELECT DISTINCT device_id FROM %s WHERE carfile_id=:carfile_id", fmt.Sprintf(blockInfoTable, area))
