...
// loadFile 加载配置
func loadFile(configPath string) ([]string, error) {
	// log.Infof("加载配置 configPath:%v", configPath)
	var config yconfig
...
var delDataCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		if err != nil {
			log.Errorf("DeleteData err:%v", err)
		}
